...
CREATE DEFINER=`root`@`localhost` PROCEDURE `filterPatientsTable` (IN `imie` VARCHAR(100), IN `nazwisko` VARCHAR(100), IN `pesel` VARCHAR(20))  BEGIN
    SET @query = 'SELECT * FROM Pacjenci WHERE 1=1';

	IF LENGTH(imie) > 0 THEN
        SET @filter = CONCAT(' AND Imie LIKE ',"'%",imie,"%'");
        SET @query = CONCAT(@query, @filter);
	END IF;

    IF LENGTH(nazwisko) > 0 THEN
        SET @filter = CONCAT(' AND Nazwisko LIKE' ,"'%",nazwisko,"%'");
        SET @query = CONCAT(@query, @filter);
    END IF;

	IF LENGTH(pesel) > 0 THEN
        SET @filter = CONCAT(' AND PESEL LIKE ',"'%",pesel,"%'");
        SET @query = CONCAT(@query, @filter);
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `filterProcedureTable` (IN `dataPr` VARCHAR(50), IN `type` VARCHAR(50), IN `orderBy` VARCHAR(10))  BEGIN
    SET @query = 'SELECT * FROM Zabiegi WHERE 1=1';

	IF LENGTH(dataPr) > 0 THEN
        SET @filter = CONCAT(' AND Data_Zabiegu = ',"'", dataPr, "'");
        SET @query = CONCAT(@query, @filter);
	END IF;

    IF LENGTH(type) > 0 THEN
        SET @filter = CONCAT(' AND Rodzaj_Zabiegu = ',"'", type,"'");
        SET @query = CONCAT(@query, @filter);
    END IF;

	IF LENGTH(orderBy) > 0 THEN
        SET @filter = CONCAT(' ORDER BY Data_Zabiegu ', orderBy);
        SET @query = CONCAT(@query, @filter);
    END IF;
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `getData` (IN `tableName` VARCHAR(100))  BEGIN
    ...
    ELSE
    	SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = "Nie ma takiej tabeli", 		MYSQL_ERRNO = 5001;
    END IF;
END$$
...
(13, 'Aleksander', 'Rajski', '71092097599', '343351595', '00-013', 'ul. Niemanna 84'),
...
(24, 'Gwalbert', 'Dudziński', '92120981954', '607181', '00-024', 'ul. Bananowa 8'),
...
(33, 'Kalina', 'Piorun', '50122425885', '888641875', '00-033', 'ul. Niemanna 85'),
...
--
ALTER TABLE `Pacjenci`
  MODIFY `ID_Pacjenta` int NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=51;
